...
class BudgetApp(tk.Tk):
    ...
    def create_new_month(self):
        ...
        if month_key in self.months_data:
            messagebox.showwarning(
                "Månad finns redan",
                f"{pretty_name} är redan skapad och kan inte skapas igen.",
                parent=self,
            )
            return

        for existing_month in self.months_data.values():
            existing_name = str(existing_month.get("name", "")).strip().lower()
            if existing_name == pretty_name.strip().lower():
                messagebox.showwarning(
                    "Månad finns redan",
                    f"{pretty_name} är redan skapad och kan inte skapas igen.",
                    parent=self,
    ...
    def activate_license_from_menu(self):
        ...
        if self._validate_license_key(key):
            ...
            self._save_settings()
            messagebox.showinfo("Licens", "Licens aktiverad! Du har nu PRO-versionen.", parent=self)
        else:
            messagebox.showerror("Fel", "Ogiltig licensnyckel.", parent=self)
